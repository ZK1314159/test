...
//                System.out.println("-------------------------------------------");
  ...
  public void putData(String tableName, String rowKey, String familyName, String[] columns, String[] values) {
    ...
    try {
      table = getTable(tableName);
      putData(table, tableName, rowKey, familyName, columns, values);
    } catch (Exception e) {
   ...
   * @param values 列值得数组
   */
  private void putData(Table table, String tableName, String rowKey, String familyName, String[] columns, String[] values) {
    try {
      //设置rowkey
   ...
   */
  @SneakyThrows
  public void batchPutOrUpdateData(String tableName, List<String> rowKey, String familyName, List<String> columns,
                                   List<String> values) {
    Table table = getTable(tableName);
    if (CollectionUtils.isEmpty(rowKey) || CollectionUtils.isEmpty(columns) || CollectionUtils.isEmpty(values)) {
      log.error("rowKey, columns, values should not be null or empty");
      return;
    }
    if (rowKey.size() != columns.size() || rowKey.size() != values.size()) {
      log.error("rowKey, columns, values should have the same size");
      return;
    }
    List<Put> puts = new ArrayList<>();
    for (int i = 0; i < rowKey.size(); i++) {
      Put put = new Put(Bytes.toBytes(rowKey.get(i)));
      put.addColumn(Bytes.toBytes(familyName), Bytes.toBytes(columns.get(i)), Bytes.toBytes(values.get(i)));
      puts.add(put);
    }
    try {
      table.put(puts);
